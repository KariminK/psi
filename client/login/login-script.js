...
const usernameRegisterInput = document.querySelector("#register-username-input");
const passwordRegisterInput = document.querySelector("#register-password-input");
const repeatPasswordInput = document.querySelector("#register-repeat-password-input");
...
const checkPassword = ()=>{
    ...
    if(matching == false){
        if(repeatedPasswordDiv.nextElementSibling.nodeName!=='P'){
            ...
            repeatedPasswordDiv.insertAdjacentElement("afterend", errorMessageParagraph);
        }
        
        classChangeArray.forEach((item)=>{
            if(item.classList.contains("matching-password")) item.classList.remove("matching-password");
        ...
        classChangeArray.forEach((item)=>{
            if(item.classList.contains("not-matching-password")) item.classList.remove("not-matching-password");
...
repeatPasswordInput.addEventListener('input', ()=>{
    if((passwordRegisterInput.value.length!==0)&&(repeatPasswordInput.value.length!==0)){
        checkPassword();
    }else {
        classChangeArray.forEach((item)=>{
...
passwordRegisterInput.addEventListener('input', ()=>{
    if((repeatPasswordInput.value.length!==0)&&(passwordRegisterInput.value.length!==0)){
        checkPassword();
    }else {
        classChangeArray.forEach((item)=>{
            item.classList.remove("matching-password");
